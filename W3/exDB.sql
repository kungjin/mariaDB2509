...
SELECT * FROM customers; -- (전체레코드의 갯수)
SELECT COUNT(*) FROM customers;
SELECT CustomerName, City, Country FROM customers; -- (선택한 컬럼들의 표기)
...
VALUES
	...
	(10, 'Ikura',	4,	8, '12 - 200 ml jars', 38.00),
...
SELECT * FROM products WHERE Price <> 30;
SELECT COUNT(*)FROM Products WHERE Price <> 30;
-- BETWEEN 50~60 사이의 값.(50부터 60까지의 값)
SELECT * FROM Products WHERE Price BETWEEN 50 AND 59;
-- <Customers 테이블의 WHERE 절> ---
-- Like는 공식이 같은 값
SELECT * FROM Customers WHERE City LIKE 'l%';
/*
  1. LIKE 는 문자열을 비교한다.
  2. 비교할 때 사용하는 규칙이 Collation (문자 집합+  정렬/비교 규칙)
  3.기본적으로 MySQL/MariaDB에서 VARCHAR, TEXT는 utf8_general_ci나 utf8mb4_general_ci 같은 _ci (case-insensitive) 콜레이션을 많이 쓴다.
     - ci = case-insensitive → 대소문자 구분 안 함.
     - 'M%'를 찾으면 M이든 m이든 다 매칭된다
*/
SELECT * FROM Customers WHERE City IN ('Berlin', 'London');

-- ORDER BY (정렬 우선순위 정의)
SELECT * FROM Products
ORDER BY ProductName ASC; -- 오름차순으로 정리> ASC 어센딩 

-- ORDER BY Country ASC, CustomerName DESC; // 기준이 2개가 맞지만, “둘을 동시에 한쪽으로 맞추는 것”이 아닌,
-- “앞 기준(오름차순)으로 같은 형식을 모은 뒤  → 뒤 기준(내림 차순)”으로 순차 정렬하는 것.

-- Null (값이 존재하지 않음)
SELECT * FROM Products WHERE ProductName IS NULL; 
SELECT * FROM Products WHERE ProductName IS NOT NULL; 
/*
1. 데이터가 아직 입력되지 않은 경우
예: 고객의 전화번호, 이메일 같은 값이 아직 수집되지 않았을 때.
2. 옵션/선택 사항인 컬럼
예: 할인율(Discount)이 없는 상품 → NULL
3. 외래 키(Foreign Key)가 아직 연결되지 않은 경우
예: 주문이 생성됐는데 배송 담당자(ShipperID)가 아직 배정되지 않은 상태.
4. 집계나 계산에서 구분할 때
COUNT(*)는 NULL 포함, COUNT(컬럼)은 NULL 제외.
5. 조건 분기 처리
NULL일 때는 기본값으로 대체해서 계산.
*/
